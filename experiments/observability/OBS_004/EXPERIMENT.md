...
Are all internal DAF spans exported correctly via the OpenTelemetry collector?

---

## Hypothesis

All loop stage spans (plan, evaluate, execute) appear in the OTEL collector with correct parent-child relationships.
...
2. Verify readiness:
   ```bash
   python -c "from experiments.observability.OBS_004.experiment import OBS_004; print('ready')"
   ```
...
```bash
python -m daf.research.runner experiments/observability/OBS_004/experiment.py
```
...
```
findings/OBS-004/OBS-004_run_NNN.log
findings/OBS-004/OBS-004_run_NNN.json
```
...
_Experiment Spec — OBS-004 — v1.0 — TonyOS Lab_
