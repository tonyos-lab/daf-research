...
Does reliability compose predictably when multiple tasks are chained in a single plan?

---

## Hypothesis

The overall success rate of a multi-task plan equals the product of individual task success rates.
...
2. Verify readiness:
   ```bash
   python -c "from experiments.reliability.REL_002.experiment import REL_002; print('ready')"
   ```
...
```bash
python -m daf.research.runner experiments/reliability/REL_002/experiment.py
```
...
```
findings/REL-002/REL-002_run_NNN.log
findings/REL-002/REL-002_run_NNN.json
```
...
_Experiment Spec — REL-002 — v1.0 — TonyOS Lab_
