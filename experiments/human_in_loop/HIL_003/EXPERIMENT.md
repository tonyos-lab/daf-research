# HIL-003 — HITL Rejection Triggers Replan
...
Does a rejected HumanReviewResponse correctly trigger the replan loop?

---

## Hypothesis

A StubHumanReviewGateway returning rejected causes the loop to replan with the rejection as violation context.
...
2. Verify readiness:
   ```bash
   python -c "from experiments.human_in_loop.HIL_003.experiment import HIL_003; print('ready')"
   ```
...
```bash
python -m daf.research.runner experiments/human_in_loop/HIL_003/experiment.py
```
...
```
findings/HIL-003/HIL-003_run_NNN.log
findings/HIL-003/HIL-003_run_NNN.json
```
...
_Experiment Spec — HIL-003 — v1.0 — TonyOS Lab_
