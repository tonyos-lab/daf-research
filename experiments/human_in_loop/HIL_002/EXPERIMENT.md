# HIL-002 — HITL Approval Flow Correctness
...
Does an approved HumanReviewResponse correctly unblock plan execution?

---

## Hypothesis

A StubHumanReviewGateway returning approved causes the gated task to execute and the workflow to complete.
...
2. Verify readiness:
   ```bash
   python -c "from experiments.human_in_loop.HIL_002.experiment import HIL_002; print('ready')"
   ```
...
```bash
python -m daf.research.runner experiments/human_in_loop/HIL_002/experiment.py
```
...
```
findings/HIL-002/HIL-002_run_NNN.log
findings/HIL-002/HIL-002_run_NNN.json
```
...
_Experiment Spec — HIL-002 — v1.0 — TonyOS Lab_
