# HIL-005 — No Gateway Auto-Rejection
...
What happens when a plan requires HITL but no gateway is configured?

---

## Hypothesis

When no HITL gateway is configured, all gated tasks are auto-rejected and the loop escalates after max_replan_attempts.
...
2. Verify readiness:
   ```bash
   python -c "from experiments.human_in_loop.HIL_005.experiment import HIL_005; print('ready')"
   ```
...
```bash
python -m daf.research.runner experiments/human_in_loop/HIL_005/experiment.py
```
...
```
findings/HIL-005/HIL-005_run_NNN.log
findings/HIL-005/HIL-005_run_NNN.json
```
...
_Experiment Spec — HIL-005 — v1.0 — TonyOS Lab_
