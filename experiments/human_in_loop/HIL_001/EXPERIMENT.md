# HIL-001 — HITL Gate Trigger Accuracy
...
Does the EvaluateStage gate exactly the tasks that match always_gate_action_classes?

---

## Hypothesis

Every task whose action_class appears in always_gate_action_classes is gated in 100% of cases, and no other task is gated.
...
2. Verify readiness:
   ```bash
   python -c "from experiments.human_in_loop.HIL_001.experiment import HIL_001; print('ready')"
   ```
...
```bash
python -m daf.research.runner experiments/human_in_loop/HIL_001/experiment.py
```
...
```
findings/HIL-001/HIL-001_run_NNN.log
findings/HIL-001/HIL-001_run_NNN.json
```
...
_Experiment Spec — HIL-001 — v1.0 — TonyOS Lab_
