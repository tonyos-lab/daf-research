# ORC-005 — Checkpoint Resume Correctness
...
After a mid-workflow interruption, does resuming from checkpoint produce the same result as an uninterrupted run?

---

## Hypothesis

A workflow resumed from a saved checkpoint produces an identical FinalResponse to one that ran uninterrupted.
...
2. Verify readiness:
   ```bash
   python -c "from experiments.orchestration.ORC_005.experiment import ORC_005; print('ready')"
   ```
...
```bash
python -m daf.research.runner experiments/orchestration/ORC_005/experiment.py
```
...
```
findings/ORC-005/ORC-005_run_NNN.log
findings/ORC-005/ORC-005_run_NNN.json
```
...
_Experiment Spec — ORC-005 — v1.0 — TonyOS Lab_
