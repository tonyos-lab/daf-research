...
How does the loop behave when the LLM returns a plan with zero sub-tasks?

---

## Hypothesis

An empty plan is evaluated and either approved (returning a completed outcome with no steps) or rejected based on PolicyMatrix configuration.
...
2. Verify readiness:
   ```bash
   python -c "from experiments.orchestration.ORC_006.experiment import ORC_006; print('ready')"
   ```
...
```bash
python -m daf.research.runner experiments/orchestration/ORC_006/experiment.py
```
...
```
findings/ORC-006/ORC-006_run_NNN.log
findings/ORC-006/ORC-006_run_NNN.json
```
...
_Experiment Spec — ORC-006 — v1.0 — TonyOS Lab_
