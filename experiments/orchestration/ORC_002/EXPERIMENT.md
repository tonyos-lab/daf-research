# ORC-002 — ScopedContext Tool Isolation
...
Are tools outside an agent's permitted set completely inaccessible within a ScopedContext?

---

## Hypothesis

Calling an unpermitted tool via ScopedContext raises ToolNotFoundError in 100% of cases.
...
2. Verify readiness:
   ```bash
   python -c "from experiments.orchestration.ORC_002.experiment import ORC_002; print('ready')"
   ```
...
```bash
python -m daf.research.runner experiments/orchestration/ORC_002/experiment.py
```
...
```
findings/ORC-002/ORC-002_run_NNN.log
findings/ORC-002/ORC-002_run_NNN.json
```
...
_Experiment Spec — ORC-002 — v1.0 — TonyOS Lab_
