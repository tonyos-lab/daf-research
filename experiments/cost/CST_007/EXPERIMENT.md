...
How does cost tracking behaviour differ between MockLLMClient and live API runs?

---

## Hypothesis

MockLLMClient cost tracking is structurally identical to live API cost tracking — the only difference is the source of the cost value.
...
2. Verify readiness:
   ```bash
   python -c "from experiments.cost.CST_007.experiment import CST_007; print('ready')"
   ```
...
```bash
python -m daf.research.runner experiments/cost/CST_007/experiment.py
```
...
```
findings/CST-007/CST-007_run_NNN.log
findings/CST-007/CST-007_run_NNN.json
```
...
_Experiment Spec — CST-007 — v1.0 — TonyOS Lab_
