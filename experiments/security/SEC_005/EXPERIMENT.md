...
Can a malicious agent inject false records into the AuditStore?

---

## Hypothesis

The AuditStore rejects duplicate audit IDs and frozen record mutation in 100% of attempts.
...
2. Verify readiness:
   ```bash
   python -c "from experiments.security.SEC_005.experiment import SEC_005; print('ready')"
   ```
...
```bash
python -m daf.research.runner experiments/security/SEC_005/experiment.py
```
...
```
findings/SEC-005/SEC-005_run_NNN.log
findings/SEC-005/SEC-005_run_NNN.json
```
...
_Experiment Spec — SEC-005 — v1.0 — TonyOS Lab_
